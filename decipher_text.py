...
class DecipherText(object): # Do not change this
    def decipher(self, ciphertext): # Do not change this
        ...
        # rlvpath3 = path.join(abspath,'OUTPUT','plaintext.txt')
        #if (len(argv) == 2):
            #rlvpath2 = argv[1]
        #if (len(argv) == 3):
            #rlvpath2 = argv[1]
        ...
        # file2 = open(rlvpath3,"w")
        file3 = open('/autograder/submission/quadgrams.txt')
        #textc = file1.read()
        # print(textc)
        textc = ciphertext
        ...
        # print(cipf)
        # print("".join(cipf))
        dictio2121 = {cipf[abfg]: abfg for abfg in range(26)}
        t = 0
        plaf2 = []
        while t < chc:
            tfc = textc[t]
            if textc[t] in cip:
                pos3 = dictio2121[textc[t]]
                tfc = plaxg[pos3]
            plaf2.append(tfc)
            t = t + 1
        plaf2 = "".join(plaf2)
        #print(plaf2)
        # file2.write(plaf2)
        deciphered_text = plaf2
        deciphered_key = "".join(cipf)
